...
Komut `KilimcininKoroglu/sshknockd` içindeki latest release’i kontrol eder, installed version ile karşılaştırır, Debian veya Ubuntu için `.deb`, CentOS, Fedora, RHEL, Rocky Linux veya AlmaLinux için `.rpm` package seçer, `dpkg -i` veya `rpm -Uvh` ile kurar ve ardından `systemctl restart sshknockd` çalıştırır.

Release asset adları package extension ve architecture içermelidir. x86_64 Debian veya Ubuntu için `.deb` asset adı `amd64` içermelidir. x86_64 RPM tabanlı sistemler için `.rpm` asset adı `amd64` içermelidir. ARM64 sistemler için asset adları `arm64` içermelidir.
